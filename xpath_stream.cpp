...
class token_redef : public token_syntax_decoder
{
  ...
  virtual void v_action (xpath_construct xc_rule, unsigned u_sub, unsigned u_variable = 0, const char* cp_literal = "") { xsp_stream->v_action (xc_rule, u_sub, u_variable, cp_literal); }
...
xpath_stream::xpath_stream (const char* cp_in) : byte_stream (cp_in) { tlp_list = new token_redef (this); }
