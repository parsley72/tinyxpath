...
class node_set
{
  ...
  void v_add_attrib_in_set (const TiXmlAttribute *XAp_attrib) { v_add_base_in_set (XAp_attrib, true); }

  /// Adds a node in the node set
  void v_add_node_in_set (const TiXmlNode *XNp_node) { v_add_base_in_set (XNp_node, false); }
  ...
  unsigned u_get_nb_node_in_set () const { return _u_nb_node; }
  ...
  int i_get_value (unsigned u_which) { return atoi (S_get_value (u_which).c_str ()); }

  /// Get the real value of a node
  double d_get_value (unsigned u_which) { return atof (S_get_value (u_which).c_str ()); }
