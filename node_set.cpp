...
/// Copy constructor
node_set::node_set (const node_set &ns2) { *this = ns2; }
...
void node_set::v_copy_selected_node_recursive_no_attrib (const TiXmlNode *XNp_root,   ///< Node whole children are to be copied
							 const char *cp_lookup)	      ///< Lookup name or NULL
{
				     ...
				     const void *vp_2)	 ///< Ptr to second element to compare
{
  ...
  i_res = i_compare_node_in_tree (p2afp_1->_XNp_root, (const TiXmlBase *) p2afp_1->_vp_node, (const TiXmlBase *) p2afp_2->_vp_node);
  switch (i_res)
  {
    case e_lower :
      return -1;
