...
class byte_stream
{
  ...
  _byte_ b_top () { return *bp_current; }
  /// Consumes one byte
  _byte_ b_pop ()
  {
    if (! o_is_valid ())
  ...
  bool o_is_valid () { return o_valid; }
  /// number of bytes still to consume
  unsigned u_remain () { return (unsigned) (bp_end - bp_current); }
  ...
  const _byte_* bp_get_backward (unsigned u_amount) { return bp_current - u_amount + 1; }
};
