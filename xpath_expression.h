...
class expression_result
{
  ...
  void v_set_root (const TiXmlNode* XNp_in) { _XNp_root = XNp_in; }

  /// Copy constructor
  expression_result (const expression_result& er_2) { *this = er_2; }
  ...
  void v_set_node_set () { _e_type = e_node_set; }
  /// Get the expression_result as a node set
  node_set* nsp_get_node_set () { return &_ns_set; }
#ifdef TINYXPATH_DEBUG
  void v_dump ();
#endif
};
